...
if (cmdarr.length < 5 && ['dev'].indexOf(cmdarr[2]) == -1) {
	console.notice('cmd like this: fibos morebps.js dev 2 6');
	process.exit(0);
}

var bp_startnum = Number(cmdarr[3]) || 2;
var bp_endnum = Number(cmdarr[4]) || 2;
if (bp_startnum < 2 || bp_startnum > bp_endnum || bp_endnum > 25) {
	console.notice('cmd like this: fibos morebps.js dev 2 25');
	process.exit(0);
}
...
describe(startev + 'more bps start', () => {
	...
	it('start bps', function() {
		...
		startccounts.forEach(function(info) {
			...
			process.start('fibos', ['./lib/morebp_start.js', startev, info.account, info.public_key, info.private_key, info.http, info.p2p]);

			coroutine.sleep(1000);

			ctx.regproducerSync(info.account, info.public_key, "http://testnet.fibos.fo", 1, {
