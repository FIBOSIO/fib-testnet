...
describe('FIBOS BIOS 启动', () => {
	...
	it('create FO', () => {
		...
		let r = ctx.excreateSync(producer_name, config.token.FO.max_supply, config.token.FO.connector_weight, config.token.FO.max_exchange, config.token.FO.reserve_supply, config.token.FO.reserve_connector_balance, {
			authorization: "eosio"
		});
		assert.equal(r.processed.action_traces[0].console, "", );
