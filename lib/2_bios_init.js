...
describe('FIBOS BIOS 启动', () => {
	...
	it('create FOD', () => {
		var ctx = fibos.contractSync("eosio.token");
		//
		var r = ctx.excreateSync(producer_name, config.token.FOD.max_supply, config.token.FOD.connector_weight, config.token.FOD.max_exchange, config.token.FOD.reserve_supply, config.token.FOD.reserve_connector_balance, 0, 0, 0, 'eosio', {
			authorization: "eosio"
		});
		assert.equal(r.processed.action_traces[0].console, "", );

		ctx.setpositionSync('0.000000 FOD@eosio', 1, 'set postion state to true', {
			authorization: "fibos"
		}); //

		assert.deepEqual(fibos.getTableRowsSync(true, "eosio.token", producer_name, "stats").rows[1], {
			"supply": "0.000000 FOD",
			"max_supply": config.token.FOD.max_supply,
			"issuer": "eosio",
			"max_exchange": "0.0000 FO",
			"connector_weight": "0.00000000000000000",
			"connector_balance": "0.0000 FO",
	...
	it('issue FOD to finance (eth2fibosgtw)', () => {
		var ctx = fibos.contractSync("eosio.token");

		let r = ctx.issueSync(systems[2].account, config.token.FOD.supply, "issue " + config.token.FOD.supply, {
			authorization: "eosio"
		});
		assert.equal(r.processed.action_traces[0].console, "", );
		...
		assert.deepEqual(fibos.getTableRowsSync(true, "eosio.token", systems[2].account, "accounts"), {
			"rows": [{
				"primary": 0,
				"balance": {
					"quantity": config.token.FOD.supply,
					"contract": "eosio"
				}
			}],
			"more": false
		...
		assert.deepEqual(fibos.getTableRowsSync(true, "eosio.token", producer_name, "stats").rows[1], {
			"supply": config.token.FOD.supply,
			"max_supply": config.token.FOD.max_supply,
			"issuer": "eosio",
			"max_exchange": "0.0000 FO",
			"connector_weight": "0.00000000000000000",
		...
		assert.deepEqual(fibos.getTableRowsSync(true, "eosio.token", systems[2].account, "accounts"), {
			"rows": [{
				"primary": 0,
				"balance": {
					"quantity": config.token.FOD.max_supply,
					"contract": "eosio"
				}
			}],
			"more": false
